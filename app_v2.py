...
with st.sidebar:
    ...
    raw_picks = st.multiselect(
        "NSE stocks — pick from the list, or type any other ticker and press Enter to add it",
        options=list(NSE_UNIVERSE.keys()),
        default=default_picks,
        format_func=lambda t: f"{t}  —  {NSE_UNIVERSE[t]}" if t in NSE_UNIVERSE else t,
        accept_new_options=True,
        help="'.NS' is appended automatically if you type a ticker without it.",
    )
    tickers = []
    for t in raw_picks:
        t = t.strip().upper()
    ...
    typed = [t for t in tickers if t not in NSE_UNIVERSE]
    if typed:
        st.caption(f"Custom tickers added: {', '.join(typed)}")
...
st.dataframe(weights_df[["Ticker", "Weight %"]].set_index("Ticker"), width='stretch')
st.download_button(
